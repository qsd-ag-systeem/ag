...
class TestDeleteFile(TestCase):
    ...
    def test_delete_file_by_name_dataset_not_exists(self, mock_dataset_exists):
        ...
        with self.assertRaises(Exception) as context:
            delete_file_by_name(self.dataset, self.file_name)

        self.assertIn("Dataset '{}' does not exist.".format(self.dataset), str(context.exception))
        

    @patch('core.delete.dataset_exists')
    @patch('core.delete.EsConnection')
    def test_delete_by_query_called_with_correct_args(self, mock_es_conn, mock_dataset_exists):
